...
function renderCards() {
  ...
  cardsArray.forEach((card, idx) => {
    // Не показываем фон и обложку как обычные карты
    if (card.name === "Фон" || card.name === "Обложка") return;
    const div = document.createElement('div');
    div.className = 'flex flex-col items-center cursor-pointer';
    div.innerHTML = `
      <img src="${card.path}" alt="${card.name}" class="card-img w-24 h-36 object-cover mb-2 ${selected.has(idx) ? 'selected' : ''}" data-idx="${idx}">
